...
[~, base_lbx, base_ubx, lbg, ubg, z0_template] = build_nlmpc_solver( ...
    predictHolizon, nx, nu, np, f_mpc, state_limits, input_limits, ...
    weights, x_target, KOZ_r, constraint);

% Wrap solver construction so each worker builds its own solver
solverConst = parallel.pool.Constant(@() build_nlmpc_solver( ...
    predictHolizon, nx, nu, np, f_mpc, state_limits, input_limits, ...
    weights, x_target, KOZ_r, constraint));

% Attitude dynamics (CasADi function) also per worker
attConst = parallel.pool.Constant(@() attitudeDynamics(simulation_dt, omega, target.InatiaMatrix()));
%% シミュレーション
...
parfor i=1:Ncase
% Local copies
solver = solverConst.Value;
att_fun = attConst.Value;
